...
if($TYPES=="SELECT_booking"){
	try {
		$Query = mysqli_query($DB,
			...
			FROM events 		
			WHERE events.orders_type_ID='1'			
			ORDER BY events.BILLCODE ASC
			");
		$DATA = [];
		$n=0;
...
else if($TYPES=="SELECT_booking_detail"){
	...
	try {
		...
		$Query2 = mysqli_query($DB,
			"SELECT events.BILLCODE,events.customer_ID,events.E_START,events.E_END,events.E_BETWEEN,events.PRICE_BEDPLUS,events.PRICE_TOTAL,events.DISCOUNT,events.DEPOSIT,events.PAY,events.OWE,events.NOTE,events.CREATED,customer.FNAME,customer.LNAME
			FROM events
			LEFT JOIN customer
