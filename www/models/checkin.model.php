...
$arr["ERROR"] = true;
if($TYPES=="SELECT_checkin"){
	try {
		$Query = mysqli_query($DB,
			"SELECT events.ID,events.BILLCODE,events.E_START,events.E_END,events.customer_ID,events.CHECKOUT,events.CREATED
			FROM events 	
			WHERE events.orders_type_ID='2'				
...
else if($TYPES=="SELECT_checkin_detail"){
	$ID = isset($_POST["ID"]) ? $_POST["ID"] : "";
	try {
		$Query = mysqli_query($DB,
			"SELECT events_detail.BILLCODE,events_detail.room_data_CODE,events_detail.PRICE
			FROM events
		...
		$Query2 = mysqli_query($DB,
			"SELECT events.BILLCODE,events.customer_ID,events.E_START,events.E_END,events.E_BETWEEN,events.BEDPLUS1,events.BEDPLUS2,events.PRICE_BEDPLUS,events.PRICE_TOTAL,events.DISCOUNT,events.DEPOSIT,events.NOTE,events.PAY,events.OWE,events.CREATED,customer.FNAME,customer.LNAME
			FROM events
			LEFT JOIN customer
